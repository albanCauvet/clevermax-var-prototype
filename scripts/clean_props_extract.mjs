...
function save(p,file){return new Promise((res,rej)=>p.pack().pipe(fs.createWriteStream(file)).on('finish',res).on('error',rej));}
function key(r,g,b){return `${r},${g},${b}`}
function clean(p){for(let i=0;i<p.data.length;i+=4){const r=p.data[i],g=p.data[i+1],b=p.data[i+2];const mx=Math.max(r,g,b),mn=Math.min(r,g,b),sat=mx-mn;if(r>170&&g>170&&b>170&&sat<16)p.data[i+3]=0;}}
...
const src='/Users/alban/Documents/New project/assets/level-01-pack-v2-technical/props_city_med_v2.png';
const out='/Users/alban/Documents/New project/assets/level-01-pack-v2-technical/props_city_med_v2.clean2.png';
const p=await load(src);clean(p);await save(p,out);const c=comps(await load(out)).filter(x=>x.pixels>600).sort((a,b)=>a.y-b.y||a.x-b.x);
console.log('count',c.length);
console.log(c.slice(0,20));
fs.writeFileSync('/Users/alban/Documents/New project/assets/level-01-pack-v2-technical/props-manifest.auto.json',JSON.stringify({file:'props_city_med_v2.clean2.png',items:c},null,2));
